			...
			<div class="col-xl-7">
				...
				<div class="card border-top border-0 border-4 border-primary">
					<div class="card-body p-5">
						...
						<hr>
						<form class="row g-3" method="POST" action="{{url('categories/edit')}}/{{$data->categoryId}}">
							@csrf
							...
							<div class="col-6">
								...
								<div class="input-group"> <span class="input-group-text bg-transparent"><i class='bx bxs-message' ></i></span>
									<input type="text" class="form-control @error('category_name') is-invalid @enderror border-start-0" id="inputEmailAddress4" placeholder="Category Name" name="category_name" value="{{old('category_name',$data->categoryName)}}" />
									@error('category_name')
									<span class="invalid-feedback" role="alert">
										<strong>{{ $message }}</strong>
									</span>
							...
							<div class="col-6">
								...
								<div class="input-group">
									...
									<select class="form-control form-select" name="food_type_id">
										...
										<option value="{{ $foodType->foodTypeId }}" @if(old('food_type_id', $data->foodTypeId) == $foodType->foodTypeId) selected @endif>{{ $foodType->foodTypeName }}</option>
										@endforeach
									</select>
								</div>
							</div>
