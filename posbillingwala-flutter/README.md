...
Point-of-sale app for restaurants, mess, and retail billing. Runs on **Android**, **iOS**, and **Web** (`pos_billingwala_v2` · v2.0.1).

Licence modules control which billing modes appear: **Fast billing**, **Dine-in**, **Takeaway**, and **Mess**. Staff roles further gate screens (for example `billing.create`, `table.view`, `report.view`).
...
Web uses a left nav (Home, Billing, Tables, Takeaway, Mess, Masters, Inventory, Reports, Settings). Phone uses the home dashboard plus Settings.
...
Unauthenticated users land on Login. After licence login, the app requires PB-PIN. Authenticated users cannot return to auth screens without logout.
...
Greeting, shop name, live clock, printer chip (Connected / USB / Network / Offline), business hours, unread notification badge.
...
Header shortcuts: **Notifications**, **Settings**, **Printer details**. Pull-to-refresh syncs catalog and sales. Opening Fast billing clears the cart and starts a POS session.
...
Same POS and Payment screens are reused for dine-in (`/tables/billing`, `/tables/payment`) and takeaway (`/takeaway/billing`, `/takeaway/payment`) without resetting the table/parcel session.
...
**Hub:** `/masters` — Categories, Subcategories, Portions, Products, Combos, Table master. Needs `product.view`.
...
Licence flags hide dine-in / takeaway / mess / fast-billing reports the shop did not buy. User-wise sales appears only when user management is on.
...
**Hub:** `/settings` — groups for billing, store, cloud, and account. Tiles respect staff permissions.
...
In-app list (FCM + local). Tapping a row deep-links (for example pending mess tokens). Home badge shows unread count.
...
Support: [info@posbillingwala.com](mailto:info@posbillingwala.com) · +91 89831 49299 · Mon–Sat 10:00–19:00 IST
