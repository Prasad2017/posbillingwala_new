...
class TakeawayParcel {
  const TakeawayParcel({required this.parcelNumber, required this.billAmount});
...
final openTakeawayParcelsProvider = Provider<List<TakeawayParcel>>((ref) {
  ...
  parcels.sort((a, b) {
    final aN =
        int.tryParse(a.parcelNumber.replaceAll(RegExp(r'[^0-9]'), '')) ?? 0;
    final bN =
        int.tryParse(b.parcelNumber.replaceAll(RegExp(r'[^0-9]'), '')) ?? 0;
