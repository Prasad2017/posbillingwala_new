...
class ProductPortionsPageState extends ConsumerState<ProductPortionsPage> {
  ...
  Future<void> productPortionsPageAdd() async {
    ...
    if (price == null) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('Enter portion price')));
      return;
    }
    setState(() => busy = true);
    try {
  ...
  Widget build(BuildContext context) {
    ...
    final masters = ref
        ...
        .maybeWhen(data: (v) => v, orElse: () => const <PortionMaster>[]);
    final selected =
        productPortionsPageSelected != null &&
            masters.any(
              (m) =>
                  m.portionMasterId ==
                  productPortionsPageSelected!.portionMasterId,
            )
        ? masters.firstWhere(
            (m) =>
                m.portionMasterId ==
              ...
              child: Column(
                ...
                children: [
                  ...
                  MasterDropdown<PortionMaster>(
                    ...
                    onChanged: (v) =>
                        setState(() => productPortionsPageSelected = v),
                  ),
              ...
              child: !loaded
                  ...
                  : Column(
                      children: [
                        for (
                          var i = 0;
                          i < productPortionsPagePortions.length;
                          i++
                        )
                          MasterListRow(
                            index: i + 1,
                            ...
                            subtitle: currency.format(
                              productPortionsPagePortions[i].portionPrice,
                            ),
                            onEdit: () {
                              ScaffoldMessenger.of(context).showSnackBar(
                                const SnackBar(
                            ...
                            onDelete: () =>
                                delete(productPortionsPagePortions[i]),
                            showDivider:
                                i < productPortionsPagePortions.length - 1,
                          ),
                      ],
                    ),
            ),
