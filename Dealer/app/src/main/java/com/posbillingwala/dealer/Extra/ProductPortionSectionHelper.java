...
import com.posbillingwala.dealer.Adapter.ProductPortionDraftAdapter;
...
/**
 * Inline optional portions on Add/Edit Product — price per product + portion master.
 */
public class ProductPortionSectionHelper {
                ...
                if (response.isSuccessful() && response.body() != null) {
                    ...
                    if (existing != null) {
                        for (ProductPortionResponse row : existing) {
                            if (row.getPortionDeletedStatus() != null
                                    && "1".equals(row.getPortionDeletedStatus())) {
                                continue;
                            }
    ...
    public void updatePriceHint() {
        if (hasPortions()) {
            portionSectionHint.setText("Portions set — product price is optional (sent as 0 if empty).");
        } else {
            portionSectionHint.setText("Product price is required when no portions. With portions, price can be 0.");
        }
    }
    ...
    private void setupPortionMasterSpinner() {
        ...
        call.enqueue(new Callback<AllApiResponse>() {
            ...
            @Override
            public void onFailure(Call<AllApiResponse> call, Throwable t) {
                Log.e("portionMasterSpinner", "" + t.getMessage());
            }
        });
    }
    ...
    /**
     * POST each draft after product save/update (server upserts by product + portionMaster).
     */
    ...
    private void saveDraftAt(String productId, int index, boolean allOk, SaveCallback callback) {
        ...
        Call<AllApiResponse> call = Api.getClient().savePortion(
                customerId,
                productId,
                draft.getPortionName() != null ? draft.getPortionName() : "",
                draft.getPortionPrice(),
                String.valueOf(draft.getSortOrder() > 0 ? draft.getSortOrder() : index + 1),
                network,
                draft.getPortionMasterId() != null ? draft.getPortionMasterId() : "");
        call.enqueue(new Callback<AllApiResponse>() {
            @Override
            public void onResponse(Call<AllApiResponse> call, Response<AllApiResponse> response) {
                boolean ok = allOk && response.isSuccessful() && response.body() != null
                        && "1".equalsIgnoreCase(response.body().getStatus());
                saveDraftAt(productId, index + 1, ok, callback);
            }
