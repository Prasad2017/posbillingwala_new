...
curl_setopt_array($ch, [
    ...
    // Decode gzip/deflate from upstream so we forward plain JSON.
    CURLOPT_ENCODING => '',
...
// Some host/malware corruption prepends binary junk before the JSON body.
// Recover the first JSON object so the marketing site can still load.
if (is_string($response) && $response !== '' && ($response[0] ?? '') !== '{' && ($response[0] ?? '') !== '[') {
