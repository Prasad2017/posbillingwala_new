...
if (!function_exists('fcm_ensure_schema')) {
    function fcm_ensure_schema($con)
    {
        ...
        if ($col === null) {
            @mysqli_query($con, "ALTER TABLE `licenses` ADD COLUMN `fcm_token` TEXT NULL AFTER `android_device_id`");
            @mysqli_query($con, "ALTER TABLE `licenses` ADD COLUMN `fcm_token_updated_at` DATETIME NULL AFTER `fcm_token`");
        }
