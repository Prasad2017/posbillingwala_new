<?php
/**
 * Upsert a table type (2/4/6 Seater etc.) with default seating capacity.
 */
...
$tableTypeName = trim($post('tableTypeName'));
$defaultCapacity = $post('defaultCapacity', '4');
$tableTypeActive = $post('tableTypeActive', '1');
$networkStatus = $post('tableTypeNetworkStatus');

if ($tableTypeName === '') {
    $response['message'] = 'tableTypeName required';
    header('Content-type: application/json; charset=utf-8');
    echo json_encode($response);
    exit;
}
...
$existing = null;
if ($networkStatus !== '') {
    $q = mysqli_query($con, "SELECT `tableTypeId` FROM `table_type`
        WHERE `licenseId`='" . $esc($userId) . "' AND `tableTypeNetworkStatus`='" . $esc($networkStatus) . "' LIMIT 1");
    if ($q) {
...
if ($existing === null) {
    $q = mysqli_query($con, "SELECT `tableTypeId` FROM `table_type`
        WHERE `licenseId`='" . $esc($userId) . "' AND $branchClause
          AND LOWER(TRIM(`tableTypeName`))=LOWER(TRIM('" . $esc($tableTypeName) . "')) LIMIT 1");
    if ($q) {
        $existing = mysqli_fetch_assoc($q);
        mysqli_free_result($q);
    }
...
$set = "`organization_id`=" . ($orgId === null || $orgId === '' ? 'NULL' : "'" . $esc($orgId) . "'") . ",
        ...
        `tableTypeName`='" . $esc($tableTypeName) . "',
        `defaultCapacity`='" . $esc($defaultCapacity) . "',
        `tableTypeActive`='" . $esc($tableTypeActive) . "',
        `tableTypeNetworkStatus`='" . $esc($networkStatus) . "'";

if ($existing !== null && isset($existing['tableTypeId'])) {
    $ok = mysqli_query($con, "UPDATE `table_type` SET $set WHERE `tableTypeId`='" . $esc($existing['tableTypeId']) . "'");
    $response['status'] = $ok ? '1' : '0';
    $response['message'] = $ok ? 'update successful!' : 'update failed!';
} else {
    $ok = mysqli_query($con, "INSERT INTO `table_type`(
        `licenseId`, `organization_id`, `branch_id`, `device_id`,
        `tableTypeName`, `defaultCapacity`, `tableTypeActive`, `tableTypeNetworkStatus`
     ) VALUES (
        '" . $esc($userId) . "',
