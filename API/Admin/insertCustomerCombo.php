<?php
/**
 * Admin: upsert combo master for a customer (mirrors POS insertCombo.php + auth).
 */
...
if ($comboName === '' || $comboPrice === '' || !is_numeric($comboPrice) || (float) $comboPrice <= 0) {
    $response['message'] = 'comboName and comboPrice (>0) are required';
    echo json_encode($response);
    exit;
}

if ($comboWithGSTPrice === '' && is_numeric($comboPrice)) {
