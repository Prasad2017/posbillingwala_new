...
public class InvoiceDiscountReport extends Fragment implements View.OnClickListener {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        ReportUiHelper.setupDetailTableHeader(binding.tableHeader,
                ...
                getString(R.string.ui_discount));
        ...
        root.setOnKeyListener((v, keyCode, event) -> {
            if (keyCode == KeyEvent.KEYCODE_BACK && event.getAction() == KeyEvent.ACTION_UP) {
                Log.i("InvoiceDiscountReport", "back pressed");
    ...
    private void loadReport(String selectedDate) {
        ...
        try {
            ...
            invoiceResponseList.addAll(posBillingWalaDatabase.getDiscountInvoiceList(selectedDate, 0, 500));
    ...
    private void bindList(String periodLabel) {
        ...
        adapter = new ReportAdapter(activity, invoiceResponseList, true);
        ...
        for (InvoiceResponse invoice : invoiceResponseList) {
            float rupees = ReportCursorHelper.discountRupees(
                    invoice.getDiscount(), invoice.getDiscountType(), invoice.getSubTotal());
            totalDiscount += rupees;
            boolean amountType = invoice.getDiscountType() != null
                    && invoice.getDiscountType().equalsIgnoreCase("Amount");
            if (amountType) {
                amountTotal += rupees;
                amountCount++;
            } else {
        ...
        ReportRankItem percent = new ReportRankItem();
        percent.setLabel(getString(R.string.discount_type_percentage));
        percent.setCount(String.valueOf(percentCount));
        percent.setTotal(String.valueOf(percentTotal));
        breakdown.add(percent);
        ReportRankItem amount = new ReportRankItem();
        amount.setLabel(getString(R.string.discount_type_amount));
        amount.setCount(String.valueOf(amountCount));
        amount.setTotal(String.valueOf(amountTotal));
        breakdown.add(amount);

        OperationalReportCharts.bindListSummary(binding, activity, invoiceResponseList.size(), totalDiscount,
                breakdown, getString(R.string.discount_wise_report),
